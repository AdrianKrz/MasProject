...
public class WyborKlientaPrezes {
    public static JFrame widok() {
        ...
        JComboBox<PrezesKlubuSportowego> comboBox = new JComboBox<>();


        for(int i = 1; i <= Osoba.getExtentSize(); i++){
            if(Osoba.rodzaj(i) instanceof PrezesKlubuSportowego)
                comboBox.addItem((PrezesKlubuSportowego) Osoba.rodzaj(i));
        }
        ...
        b2.addActionListener( s -> {
            ...
            DaneDoRezerwacji.setPrezes((PrezesKlubuSportowego) Osoba.rodzaj(ids));
            f.setVisible(false);

            RezerwacjaKlub.widok().setVisible(true);
